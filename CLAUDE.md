...
- App icons: edit the SVG at `data/icons/hicolor/scalable/apps/com.fiberelements.Cloudy.svg`, then regenerate the PNG sizes with `magick` (the only raster tool available here — no rsvg/inkscape) into `data/icons/hicolor/<size>x<size>/apps/` (sizes 48/64/128/256/512, all wired into `data/icons/meson.build`).
